...
class Country {
    ...
    async update(data) {
        ...
        const response = await db.query("UPDATE country SET name = $1, capital = $2, population = $3 WHERE name = $4 RETURNING *;", [name, capital, population, this.name])
        if (response.rows.length !=1) {
            throw Error("Unable to update country")
        }
        return new Country(response.rows[0])
    }
